...
このように設定しておくことで、テンプレートタグ`{% raw %}{% load static %}{% endraw %}`で`static/`ディレクトリへのパスを読み込めるようになります。
...
`base.html`の冒頭で、テンプレートタグ`{% raw %}{% load static %}{% endraw %}`と記述することで、このテンプレート内から`static/`ディレクトリのファイルを参照することができるようになります。また、このテンプレートでBootstrapを利用するため、下記のテンプレートタグを記述しています。
- `{% raw %}{% load django_bootstrap5 %}{% endraw %}`
- `{% raw %}{% bootstrap_css %}{% endraw %}`
- `{% raw %}{% bootstrap_javascript %}{% endraw %}`
