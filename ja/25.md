...
推薦スロットから、例えば「カツオ」をクリックしてみましょう。エラーメッセージが表示されますが、現時点では無視して「OK」ボタンをクリックしてください。すると、上図のように、対象アイテムであるカツオに関する詳細情報と共に、「カツオが好きな人はこんな寿司も好きです。」と表示されました。これはアイテム類似度に基づく推薦リストを表します。対象アイテムの詳細情報の部分には、アイテム名とカテゴリ、評価値を表す星評価が表示されています。星評価にマウスカーソルを重ねると、星がハイライト表示されます。本来であれば、星をクリックすることで対象アイテムに対する評価値を登録できるのですが、現時点では、エラーメッセージが表示されます。対象アイテムの評価値の取得と登録については、ユーザログイン機能を実装した後で実装しましょう。
