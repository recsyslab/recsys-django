...
ここで、星評価の画像ファイル`star0.png`、`star1.png`、`star2.png`と、メインページへ戻るための「戻る」ボタンの画像ファイル`return.png`を用意し、`recsys_django/static/img/`ディレクトリに配置しておきます。[recsyslab/recsys-django/src/recsys_django/static/img/](https://github.com/recsyslab/recsys-django/tree/main/src/recsys_django/static/img/)に各画像のサンプルファイルを置いています。
