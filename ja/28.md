...
リスト5: `recsys_django/online/index.html`
```js
    ...（略）...
    {# --- js --- #}
    ...（略）...
...
[ログイン機能の実装](27.md)で作成したアカウントでログインした状態で、アイテム詳細ページから評価値を表す星評価をクリックしてみてください。例えば、カツオに対して評価値`3`を設定してみましょう。

![インタフェース](images/interface28a.png)

すると、評価値が確定しました。一旦、メインページに戻って、再度カツオの詳細ページを開くと、評価値が`3`で保存されていることがわかります。
