...
これで、カスタムユーザのモデルが定義できました。`manage.py`があるディレクトリにいることを確認したうえで、下記のコマンドを実行し、マイグレーションファイルを作成します。本チュートリアルでは、データベースに接続するユーザ名を`rsl`としておきます。【パスワード】には、ユーザ`rsl`でデータベースに接続するためのパスワードとして任意のパスワードを入力してください。
